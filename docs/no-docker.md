...
No Dockerfile, no container, no local engine: ship a plain binary (or a release tarball) and run it as a bare systemd process. Same commands, same blue-green flip, same everything as the [Docker quick start](quick-start.md) — the artifact is the only difference. This is the right path when your runtime fits in a single file: Go, Rust, Zig, or anything that cross-compiles clean.

You opt in by stating `build` and `artifact` in deploy.json — hadi can infer a Docker build from a Dockerfile, but only you know how to build your binary.
...
Two rules make any service hadi-deployable: read your port from an env var (`PORT` unless `run.port_env` says otherwise), and answer a health path.
...
That's the whole deployment surface. `hosts` points straight at your box (swap in DNS discovery later); `entry.domain` means public HTTPS with automatic certificates. The two lines that make this a binary deploy are `build` and `artifact` — stating them is what tells hadi not to look for a Dockerfile.
...
- [quick-start.md](quick-start.md): the same walkthrough for the default path — a Dockerfile
